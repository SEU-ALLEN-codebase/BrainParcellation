...
def plot_region_feature_in_ccf_space(mefile, rname='MOB', r316=False, flipLR=True, savefig=True, elev=30, azim=-60):
    ...
    if rname in ('l2/3', 'l4'):
        # get all the L2/3 cortical neurons
        all_regions = np.unique(df.region_name_r671[~df.region_name_r671.isna()])
        rname = [reg for reg in all_regions if reg.endswith(rname[1:])]
    ...
    if savefig:
        ...
        plt.savefig(f"{out_prefix}_features_ccf_e{elev}_a{azim}.png", bbox_inches='tight')
        plt.close()
    else:
        plt.show()
...
def plot_region_feature_sections(mefile, rname='MOB', r316=False, flipLR=True, thickX2=10, axid=2):
    ...
    # Do nearest neighbor smoothing
    #topk = 5
    #A1 = kneighbors_graph(coords, n_neighbors=topk, include_self=False, mode='distance', metric='euclidean')
    #yc, xc = A1.nonzero()
    #import ipdb; ipdb.set_trace()
    #Ab = community_memberships[xc].reshape(-1, topk)
    ...
    show_intra_boundary = True
    if show_intra_boundary:
        mm = mask.copy()
        mm[rmask == 0] = 0
        sub_mask = mm[zmin:zmax+1, ymin:ymax+1, xmin:xmax+1]
    ...
    for sid in range(0, dmax-dmin-thickX2-1, thickX2*2):
        ...
        figname = f'{out_prefix}_axid{axid}_section{sid:03d}.png'
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        rname = ['MOp2/3']
        plot_region_feature_sections(mefile, rname, axid=1, thickX2=10)
        for elev in range(0, 90+1, 30):
            for azim in range(0, 90+1, 30):
                plot_region_feature_in_ccf_space(mefile, rname, elev=elev, azim=azim)
