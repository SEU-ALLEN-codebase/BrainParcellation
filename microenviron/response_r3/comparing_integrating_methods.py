...
def calc_eigenvalues(data_file):
    ...
    if 1:
        ...
        cumsum_df = cumsum_df / 24

        cumsum_df_ablation = cumsum_df.iloc[2:].copy()
        cumsum_df_ablation['Top-K'] = [int(name.split('_')[0][4:]) for name in cumsum_df_ablation.index]
        cumsum_df_ablation['Radius'] = [int(name.split('_')[1][1:]) for name in cumsum_df_ablation.index]
        cumsum_df_ablation.drop(['v1', 'v2', 'v4', 'v5'], axis=1, inplace=True)
        ...
        # rename
        sns.lineplot(data=cumsum_df_ablation, x='Radius', y='v3', 
                     hue='Top-K', marker='o', lw=2, markersize=12,
                     alpha=1.0)
        #plt.title('Cumulative variance comparison')
        #plt.xlim(0.5, 5.5)
        #plt.ylim(5, 18)
        plt.xlabel('Radius (µm)')
        plt.ylabel('Sum of variance ratios of first 3 PCs')
        ...
        plt.legend(labelspacing=0.2, handletextpad=0.2,
                       ...
                       title='Number of nearby neurons')
        ...
        plt.tight_layout()
        plt.savefig(f'cumulative_variances_various_conditions.png', dpi=300)
        plt.close()
        print()
    ...
    if 1:
        ...
        imethods = [key for key in data.keys() if key != 'dendrite-only']
        ...
        median_scores = df_scores.iloc[:,2:-1].median(axis=0)
        median_scores = median_scores.to_frame(name='silhouette_score')
        median_scores['Top-K'] = [int(name.split('_')[0][4:]) for name in median_scores.index]
        median_scores['Radius'] = [int(name.split('_')[1][1:]) for name in median_scores.index]
        

        # plotting
        sns.set_theme(style='ticks', font_scale=1.8)
        plt.figure(figsize=(8,6))
        sns.lineplot(data=median_scores, x='Radius', y='silhouette_score',
                     hue='Top-K', marker='o', lw=2, markersize=12,
                     alpha=1.0)
        #plt.title('Cumulative variance comparison')
        #plt.xlim(0.5, 5.5)
        ...
        ax.spines['bottom'].set_linewidth(2)
        plt.tight_layout()
        plt.savefig(f'silhouette_scores_across_conditions.png', dpi=300)
        plt.close()
        print()
