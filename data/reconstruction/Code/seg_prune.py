...
def get_anchors(morph: Morphology, ind: list, dist: float, spacing=(1, 1, 1), epsilon=1e-7, gap_thr_ratio=1, step_size=0.5, adjust_center=False):
    """
    ...
    if adjust_center:
        ...
        for pts in pts_ch:
            ...
            while j < len(pts):
                ...
                j += 1
            if len(pp) > 1:
                f = interp1d(dist_cum, np.array(pp), 'quadratic' if len(pp) > 2 else 'linear', 0, fill_value='extrapolate')
                interp_ch.append(f)
        step = step_size
        while step <= dist:
...
class SegmentPruning:
    ...
    def __init__(self, spacing=(1, 1, 1), soma_radius=15, anchor_dist=20, anchor_tolerance=10, near_anchor=5, gap_thr_ratio=1):
        ...
        for i in other_soma:
            ...
            for j, p in enumerate(pts):
                if j == 0 or j == len(pts) - 1 or np.linalg.norm((p[2:5] - cs) * self.spacing) <= self.soma_radius:
                    ...
                    gray2.append(0)
                    continue
                center, anchor_p, anchor_ch = get_anchors(tmp_morph, [p[0]], self.anchor_dist, self.spacing, gap_thr_ratio=self.gap_thr_ratio)[:3]
    ...
    def branch_prune(self, morph, angle_thr=80):
        ...
        for n in morph.bifurcation | morph.multifurcation:
            ...
            center, anchor_p, anchor_ch, protrude, com_node, pts_p, pts_ch, rad_p, rad_ch = \
                get_anchors(morph, [n], self.anchor_dist, self.spacing, adjust_center=True, gap_thr_ratio=self.gap_thr_ratio)
            near_p, near_ch = \
    ...
    def crossing_prune(self, morph, dist_thr=5, angle_thr1=80, angle_thr2=120, para_angle_thr=135):
        ...
        for x in crossings:
            ...
            center, anchor_p, anchor_ch, protrude = \
                get_anchors(morph, x, self.anchor_dist, self.spacing, adjust_center=True, gap_thr_ratio=self.gap_thr_ratio)[:4]
            near_p, near_ch = \
...
def main(swc):
    outpath = sys.argv[2] / Path(swc).relative_to(sys.argv[3])
    # outpath = Path('test.swc')
    if not overwrite and outpath.exists():
        return

    # prune
    try:
        res = res_xy.at[int(Path(swc).parent.name)]
        if np.isnan(res):
            res = .25
        ...
        with TemporaryDirectory() as td:
            tp = Path(td) / 't.swc'
            subprocess.run(['xvfb-run', '-a', '/public/home/vkzohj/software/vaa3d/start_vaa3d.sh', '-x', 
                           'pruning_swc_simple', '-f', 'pruning_iterative', '-i', swc, '-o', 
                           tp, '-p', str(int(20 / res))], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        with HidePrint(debug):
            tree = swc_handler.prune(tree, 
                                     ...
                                     pruner.crossing_prune(morph, dist_thr=10, angle_thr1=45, angle_thr2=90, para_angle_thr=135) |
                                     (pruner.soma_prune(Morphology(tree_for_soma), candidate_radius=int(3 / res), area_thr=int(200 / res), non_detect_radius=int(20 / res)) &
...
if __name__ == '__main__':
    ...
    res_xy = pd.read_csv(wkdir / 'supp.csv', index_col=1, header=0).iloc[:, 3]
    # main('app2/191798/4550_9870_8517.swc')
    # exit()
